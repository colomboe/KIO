...
data class EnvAccess<ENV, OUT>(val accessF: suspend (ENV) -> Eval<ENV, OUT>): Eval<ENV, OUT>()
data class FlatMap<ENV, IN, OUT>(val flatMapF: suspend (IN) -> Eval<ENV, OUT>, val prev: Eval<ENV, IN>): Eval<ENV, OUT>()

object EvalFn {

    fun <ENV, OUT> eager(a: OUT) = Eager<ENV, OUT>(a)
    fun <ENV, OUT> lazy(f: suspend CoroutineScope.() -> OUT) = Lazy<ENV, OUT> { coroutineScope(f) }
    fun <ENV, OUT> evalAccessEnv(f: suspend CoroutineScope.(ENV) -> Eval<ENV, OUT>) = EnvAccess { env: ENV -> coroutineScope { f(env) } }
    fun <ENV, OUT> laterEnv(f: suspend CoroutineScope.(ENV) -> OUT) = evalAccessEnv<ENV, OUT> { env -> eager(f(env)) }

    fun <ENV, IN, OUT> Eval<ENV, IN>.evalMap(f: (IN) -> OUT): Eval<ENV, OUT> = FlatMap({ Eager<ENV, OUT>(f(it)) }, this)
    fun <ENV, IN, OUT> Eval<ENV, IN>.evalFlatMap(f: suspend (IN) -> Eval<ENV, OUT>): Eval<ENV, OUT> = FlatMap(f, this)
